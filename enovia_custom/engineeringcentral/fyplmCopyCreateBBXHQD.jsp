...
<script language="javascript" src="scripts/emxUIFormUtil.js"></script>
	...
	<script type="text/javascript">
		function alertSelectedOneClxhqd(){
			alert('<emxUtil:i18nScript localize="i18nId">emxEngineeringCentral.Message.SelectedOneClxhqd</emxUtil:i18nScript>');
			top.close();
		}
...
<%
    Logger logger = LogManager.getLogger("fyplmCopyCreateBBXHQD.jsp");
    logger.info("Enter the fyplmCopyCreateBBXHQD.jsp.");
    String objectId = "";
    StringList strList = new StringList();
    try {
        String sSelectIds[] = emxGetParameterValues(request, "emxTableRowId");
        if(sSelectIds.length != 1){
        	out.println("<script>alertSelectedOneClxhqd();</script>");
            return;
        }
      ...
      //System.out.println("sObjectId111========"+objectId);
        if (objectId.indexOf("|") != -1) {
        	...
        	DomainObject objClxhqd = DomainObject.newInstance(context,objectId);
	        String strCurrent = objClxhqd.getInfo(context, DomainObject.SELECT_CURRENT);
	        if(!strCurrent.equals("Release")){
            	out.println("<script>alertSelectedReleaseClxhqd()</script>");
            	return;
            }
	        
	        String mqlStr = "print bus " + objectId + " select to[FYPLM LP Clxhqd].from.id dump";
            String sLPobjectId = MqlUtil.mqlCommand(context, mqlStr);
	System.out.println("sLPobjectId: " + sLPobjectId);
	        
        String url = request.getContextPath()+"/common/emxCreate.jsp?objectId="+sLPobjectId+"&type=type_FYPLMBBXHQD&policy=policy_FYPLMBBXHQD&form=type_createBBXHQDByBMRAndLPForm&mode=edit&HelpMarker=emxhelpdocumenteditdetails&submitAction=refreshCaller&formHeader=emxComponents.Header.FYPLMCreateBBXHQDByBMRAndLPForm&Export=False&findMxLink=false&suiteKey=Components&StringResourceFileId=emxComponentsStringResource&nameField=autoname&showPolicy=false&relationship=relationship_FYPLMLPBBXHQD&postProcessJPO=fyplmBBXHQD:copyCreateBBXHQDPostProcess";
